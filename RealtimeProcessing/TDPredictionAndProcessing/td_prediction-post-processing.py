...
def compute_confidence_interval(values, mean, distribution, confidence_level):
    if distribution == 'normal':
        ...
        # Calculate confidence interval using mean ± 2 * standard deviation
    ...
    while True:
        ...
        if len(row) == 0:
            ...
            if len(previous_values) == 3 and previous_values[0] == previous_values[1] == previous_values[2]:
                print(f"Prediction Failed - Infinite Loop: {id_step} - {end_step}")
                state = f'Prediction Failed - Infinite Loop - {id_step}'
                break


            # print(f"End Reached 1: {id_step} - {end_step}")
            if id_step == end_step:
                print(f"Predict to Final Berth (1): {id_step} - {end_step}")
                state = f'Predict to Final Berth - {id_step}'
                break
            elif id_step[:2] == end_step[:2]:
                print(f"Predict to Final Area (1): {id_step} - {end_step}")
                state = f'Predict to Final Area - {id_step} - {end_step}'
                break
            else:
                    ...
                    if new_id:
                        ...
                        if len(transition) >= 1:
                            ...
                            if len(transition) >= 1:
                                ...
                                print(f"Prediction Failed (1-3): {id_step} - {end_step}")
                                state = f'Prediction Failed - {id_step}'
                                break  
            ...
            if len(row) == 1:
                ...
                if len(row) == 0:
                    if id_step == end_step:
                        print(f"Predict to Final Berth (3): {id_step} - {end_step}")
                        state = f'Predict to Final Berth - {id_step}'
                        break
                    elif id_step[:2] == end_step[:2]:
                        print(f"LPredict to Final Area (3): {id_step} - {end_step}")
                        state = f'Predict to Final Area - {id_step} - {end_step}'
                        break
                    else:
                        print(f"Prediction Failed (3): {id_step} - {end_step}")
                        state = f'Prediction Failed - {id_step}'
                        break
                else:
                    pass
                ...
                if id_step == end_step:
                    print(f"Predict to Final Berth (2): {id_step} - {end_step}")
                    state = f'Predict to Final Berth - {id_step}'
                    break
                elif id_step[:2] == end_step[:2]:
                    print(f"Predict to Final Area (2): {id_step} - {end_step}")
                    state = f'Predict to Final Area - {id_step} - {end_step}'
                    break
                else:
                    print(f"Prediction Failed (2): {id_step} - {end_step}")
                    state = f'Prediction Failed - {id_step}'
                    break

        if id_step == end_step:
            print(f"Predict to Final Berth (4): {id_step} - {end_step}")
            state = f'Predict to Final Berth - {id_step}'
            break
...
def process_ids(ids):
    ...
    for id in ids:
        ...
        compare_df, results, train = predict_and_compare_service(id, df_predict, transitions, berth_to_loc)
...
def get_archive_trains(mongo_archive):
    resp = list(mongo_archive.find({},{"_id": 0, "id": 1}).limit(16000))
    trains = []
    for r in resp:
        trains.append(r['id'])
    return trains
