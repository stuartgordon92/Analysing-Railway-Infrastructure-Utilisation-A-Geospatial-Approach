...
def get_today(sched, ex_date):
    ...
    todays_tt['timetable_id'] = todays_tt['id']
...
def mongo_upload_multi(mongo, tbl_name, df):
    ...
    x = mongo_tbl.insert_many(df.to_dict('records'), ordered=False)
    return x
...
def mongo_upload_archive(mongo, tbl_name, data):
    ...
    x = mongo_tbl.insert_many(data, ordered=False)
    return x
...
def m_upload(data, url, db):
    mongo_database = mongo(url, db)
    resp = mongo_upload_multi(mongo_database, 'timetable', data)
    print(resp.inserted_ids)
    return resp

def m_archive(archive_date, url, db):
    mongo_database = mongo(url, db)
...
if __name__ == '__main__':
    ...
    todays_timetable_mp['timetable_id'] = todays_timetable_mp['id']
    todays_timetable_mp = todays_timetable_mp[['id', 'timetable_id', 'CIF_train_uid', 'signalling_id', 'running_date',
        ...
        ]].rename(columns={'timetable_id': '_id'})
    ...
    print(f"{datetime.now()} - Duplicate timetable IDs")    
    # Find duplicate rows
    df_duplicates = todays_timetable_mp[todays_timetable_mp['_id'].duplicated()]
    print(df_duplicates)


    todays_timetable_mp = todays_timetable_mp.sort_values(by=['_id', 'origin_departure_timestamp']).drop_duplicates(subset=['_id'], keep='last')
